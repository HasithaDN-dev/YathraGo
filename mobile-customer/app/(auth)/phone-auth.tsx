...
// Import new centralized API function
...
export default function PhoneAuthScreen() {
  ...
  // --- All your excellent validation and formatting logic remains unchanged ---
  const handlePhoneChange = (text: string) => {
    // Remove all non-digits
    const cleaned = text.replace(/\D/g, '');
    // Limit to 10 digits max for display
    setPhoneNumber(cleaned.substring(0, 10));
  };
  ...
  const handleSendOTP = async () => {
    ...
    try {
      // **THE CHANGE**: We now call our new, centralized API function.
      await sendOtpApi(apiPhoneNumber);
      ...
      Alert.alert('Error Sending OTP', message);
    } finally {
      setIsLoading(false);
    }
  };
